...
const randomDelay = (min = 50, max = 150) => {
...
// Get crawl status
app.get('/api/crawl/:sessionId/status', (req, res) => {
  const crawler = crawlSessions.get(req.params.sessionId);
  if (!crawler) {
...
// Get pages (paginated)
app.get('/api/crawl/:sessionId/pages', (req, res) => {
  const crawler = crawlSessions.get(req.params.sessionId);
  if (!crawler) {
    return res.status(404).json({ error: 'Session not found' });
  }
...
// Download CSV
app.get('/api/crawl/:sessionId/download', (req, res) => {
  const crawler = crawlSessions.get(req.params.sessionId);
  if (!crawler) {
    return res.status(404).json({ error: 'Session not found' });
  }
...
// Stop crawling
app.post('/api/crawl/:sessionId/stop', (req, res) => {
  const crawler = crawlSessions.get(req.params.sessionId);
  if (!crawler) {
    return res.status(404).json({ error: 'Session not found' });
  }
